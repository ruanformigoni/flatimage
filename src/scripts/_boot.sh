...
if ! test -t 1 || ! test 2; then
  notify-send "Using custom stdout/stderr to ${FIM_DIR_MOUNT}.log" || true
...
function main()
{
  ...
  declare -a ext_tools=(
    ...
    "nice" "nl" "nohup" "nproc" "numfmt" "od" "overlayfs" "paste" "pathchk" "pr" "printenv" "printf"
    "ptx" "pwd" "readlink" "realpath" "rm" "rmdir" "runcon" "seq" "sha1sum" "sha224sum" "sha256sum"
    "sha384sum" "sha512sum" "shred" "shuf" "sleep" "sort" "split" "stat" "stty" "sum" "sync" "tac"
    "tail" "tee" "test" "timeout" "touch" "tr" "true" "truncate" "tsort" "uname" "unexpand" "uniq"
    "unlink" "uptime" "users" "vdir" "wc" "who" "whoami" "yes" "resize2fs" "mke2fs" "lsof"
  )
